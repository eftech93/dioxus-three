...
use dioxus_three::{ModelFormat, ShaderPreset, ModelConfig};
...
fn SelectedModelControls(
    ...
    let color = model.as_ref().map(|m| m.config.color.clone()).unwrap_or_else(|| "#ff6b6b".to_string());
